...
def main() :
    ...
    if len(sys.argv) == 1 :
        ...
        if args.auth == False and args.follow_list == False and args.follow_list_download == False :
            if args.File == None :
                real_main(args.url, langwithindex, args.quality_mode, args.destination, args.max_threads, args.name, args.zip_name, args.selchap)
            else :
                file_main(args.File, langwithindex, args.quality_mode, args.destination, args.max_threads, args.zip_name, args.selchap)
        elif args.follow_list_download :
            token = auth()
            file = get_follow_list(token)
            file_main(file, langwithindex, args.quality_mode, args.destination, args.max_threads, args.zip_name, args.selchap)
